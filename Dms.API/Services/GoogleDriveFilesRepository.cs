...
namespace Dms.API.Services
{
    public class GoogleDriveFilesRepository : IGoogleDriveFilesRepository
    {
        ...
        public GoogleDriveFilesRepository(IDataBaseRepository IDataBaseRepository, IWebHostEnvironment env)
        {
        ...
        public async Task<DriveService> GetService()
        {
            ...
            var resultConfig = await _IDataBaseRepository.GetConfiguration();
                ...
                if (requestResult.Files != null && requestResult.Files.Count > 0)
                {
                    ...
                    foreach (var file in requestResult.Files)
                    {
                        FileResponse File = new FileResponse
                        {
                            ...
                            capabilities_can_download = file.Capabilities.CanDownload,
                            capabilities_can_list_children = file.Capabilities.CanListChildren,
                            file_download_link = !string.IsNullOrEmpty(Path.GetFileName(file.Name)) ? $"https://dev-api.niftytrader.in:83/api/Storage/DownloadFile/{file.Id}" : ""
                        };
        ...
        public async Task<ResultObjectDTO<List<FileResponse>>> SearchFiles(FileRequestApiModel requestApiModel)
        {
            ...
            try
            {
                ...
                var searchFiles = requestResult.Files.Where(d => d.Name.ToLower().Contains(requestApiModel.file_attribute.ToLower()) || d.Description != null && d.Description.ToLower().Contains(requestApiModel.file_attribute.ToLower())).ToList();
                FileList.ResultData = new List<FileResponse>();
                if (searchFiles != null && searchFiles.Count > 0)
                {
                    ...
                    foreach (var file in searchFiles)
                    {
                        FileResponse File = new FileResponse
                        {
                            ...
                            capabilities_can_download = file.Capabilities.CanDownload,
                            capabilities_can_list_children = file.Capabilities.CanListChildren,
                            file_download_link = !string.IsNullOrEmpty(Path.GetExtension(file.Name)) ? $"https://dev-api.niftytrader.in:83/api/Storage/DownloadFile/{file.Id}" : ""
                        };
                        FileList.ResultData.Add(File);
                    }
                }
        ...
        public async Task<ResultObjectDTO<List<FileResponse>>> GetDriveFolders()
        {
            ResultObjectDTO<List<FileResponse>> FileList = new ResultObjectDTO<List<FileResponse>>();
            try
            {
        ...
        public async Task<ResultObjectDTO<string>> CreateFolder(FileRequestApiModel fileRequest)
        {
            ResultObjectDTO<string> resultDrive = new ResultObjectDTO<string>();
            try
            {
                ...
                var FileMetaData = new Google.Apis.Drive.v3.Data.File();
                FileMetaData.Name = fileRequest.file_name;
                ...
                resultDrive.Result = ResultType.Success;
                resultDrive.ResultMessage = ResultType.Success.ToString();
                resultDrive.ResultData = file.Id;
        ...
        public async Task<ResultObjectDTO<string>> DeleteFile(string fileId)
        {
            ResultObjectDTO<string> resultDrive = new ResultObjectDTO<string>();
            try
            {
        ...
        public async Task<ResultObjectDTO<FileResponse>> GetFileInfo(string fileId)
        {
            ...
            try
            {
                ...
                fileRequest.Fields = "webViewLink, capabilities";
                ...
                if (file != null)
                {
                    ...
                    fileResult.ResultData.capabilities_can_download = file.Capabilities.CanDownload;

                }
                else
                {
                    fileResult.Result = ResultType.Error;
                    fileResult.ResultMessage = "No Record Found";
                }
